...
public class SessionTokenFilter implements Filter {
    ...
    //日志文件
    private static Logger logger = Logger.getLogger(SessionTokenFilter.class);
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(serverName.equals("login")){//登录接口不需要校验
        ...
        if(TokenUtils.check(token)){
            ...
            servletResponse.setContentType("text/json;charset=UTF-8");
            servletResponse.setCharacterEncoding("UTF-8");
            servletResponse.getWriter().println(" {\"code\":\"-2\",\"message\":\"session invalid,login again\"}");
            return;
        }
